	...
	public class CsvReader
	{
		...
		private bool FillBuffer()
		{
			...
			if (toRead >= bufferLoadThreshold)
			{
				...
				{
					int read0 = ReadBlockAndCheckEof(buffer, freeStart, buffer.Length - freeStart, ref eof);
					actualBufferLen += read0; 
					Console.WriteLine($"0\t{freeStart}\t{buffer.Length - freeStart}\t{read0}");
					if (lineStartPos > 0)
					{
						int read1 = ReadBlockAndCheckEof(buffer, 0, lineStartPos, ref eof);
						actualBufferLen += read1;
						Console.WriteLine($"1\t0\t{lineStartPos}\t{read1}");
					}
				}
				else
				{
					int read2 = ReadBlockAndCheckEof(buffer, freeStart, toRead, ref eof);
					actualBufferLen += read2;
					Console.WriteLine($"2\t{freeStart}\t{toRead}\t{read2}");
				}
			}
			return eof;
		}
